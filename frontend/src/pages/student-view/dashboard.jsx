...
import { Menu, Settings, LogOut, User, Home, Clipboard, Stamp } from "lucide-react";
  ...
  return (
    <div className="flex w-screen h-screen bg-white">
      ...
      <aside className={`fixed  md:relative w-80 px-7 lg:w-1/5 bg-gray-50 text-black shadow-xl flex flex-col h-screen transition-transform duration-300 z-50 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:flex`}>
        {/* Logo */}
        <div className="flex items-center justify-between h-20 px-4 border-b border-gray-300 md:justify-center">
      ...
      <main className="flex-1 p-2 bg-white">
        {/* Header */}
        <header className="flex items-center justify-between bg-white px-4 py-2 shadow-md md:shadow-none">
        ...
        {/* Main Content Area */}
        <div className="bg-white h-full rounded-lg p-1 md:px-6 overflow-scroll">
          <Outlet />
        </div>
      </main>
    </div>
